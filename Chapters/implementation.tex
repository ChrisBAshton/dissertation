...
On the one hand, I could write simple code in the controllers that only had to worry about getting an object's ID, meaning I could chain method calls together; I could query objects and pass returned IDs into subsequent constructors to quickly and easily get the objects I needed.

On the other hand, the models were made impure due to the database connection object. Though the models were somewhat decoupled from the database - all SQL queries were encapsulated in the intermediary database querying objects - they still had to know about the existence of the database and could not exist independently of a persistence layer. The original design also made the models quite difficult and slow to test.

Database querying was not restricted to the constructors; any side-effect functions were also tightly coupled to the database. For example:
...
Again, this muddled the responsibilities of the model and made it difficult to test and to maintain. It was tightly coupled to the underlying table representation, and became fiddly when having multiple functions that change different fields: functions had to know how to update individual fields related to the object.

\section{Refactoring to pure models}
\label{section:refactoringToPureModels}

The decision was made to change models to take an array of data rather than a database row ID, fully decoupling them from the database. The array of data may or may not have come from a database and the models have no concept of persistence.
...
Extracting the database retrieval out of the model also meant extracting the database \emph{changes} out of the model. When moving this behaviour into the controllers, I decided to encapsulate all updates inside one update method that corresponds to the model:
...
\item \textbf{DBQuery.php} - defines miscellaneous queries that would not be efficient or cohesive if encapsulated elsewhere, such as \lstinline{getAllDisputes()}, which retrieves all disputes associated with a given account.
\end{itemize}

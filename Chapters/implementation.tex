...
Finally, I utilised the CodeClimate code-monitoring service for an automated second opinion on my code, as well as the Gemnasium dependency-monitoring service to warn me whenever my project's dependencies were out of date. All of these third-party services were well worth spending time configuring and have ensured that the core software is always of a high quality.
...
\item \textbf{DBQuery.php} - defines miscellaneous queries that would not be efficient or cohesive if encapsulated elsewhere. For example, \lstinline{getAllDisputes()} retrieves all disputes associated with a given account.
\end{itemize}
\end{minipage}
...
Note that the module declaration function is different to the module definition function. The former is always executed, as it declares the module to the system. It \emph{contains} the module definition as an anonymous function which is only executed if the module is `active'.

...
On the one hand, I could write nice code in the controllers that only had to worry about getting an object's ID. This meant that I could chain method calls together, passing returned IDs into subsequent constructors to quickly and easily get the objects I needed.

On the other hand, the models were made impure due to the database connection object. Though the model was somewhat decoupled from the database - all SQL queries were encapsulated in the intermediary database querying objects - it still had to know about the existence of the database. It could not exist independently of a persistence layer. It also made the models quite difficult and slow to test.

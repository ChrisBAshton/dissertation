...
Unit tests verify that the publicly available methods of a class work as expected, both when accessed correctly and \emph{incorrectly}. The nature of unit tests should be that they are loaded into memory and are very quick to run. ``Unit tests focus just on the class or method at hand. They run entirely in memory, which makes them very fast. Depending on your platform, your testing tool should be able to run at least 100 unit tests per second." [REF] % http://www.jamesshore.com/Agile-Book/test_driven_development.html

Some of the unit tests in SmartResolution are not `true' unit tests, as they rely on a database connection. According to Michael Feathers [REF], ``A test is not a unit test if it talks to the database, it communicates across the network, it touches the file system or it can't run at the same time as any of your other unit tests.'' % http://www.artima.com/weblogs/viewpost.jsp?thread=126923
...
This seemed laborious, error-prone and messy. Instead, I defined a custom exception function [REF] which throws an exception, but also rolls back any existing transactions. % https://github.com/ChrisBAshton/smartresolution/blob/b33f30064048d2dc6c84d72736dcdacb02d293b9/webapp/core/model/Utils.php#L14-L25
...
Now that the root of the problem had been fixed, unit tests could be run without clearing the database in between each test. This led to big improvements in test speed: at this stage, I could now run 112 unit tests in just 34 seconds [REF]. This is 3.29 unit tests per second (UTPS), compared with 1.58 UTPS before the optimisation. % https://travis-ci.org/ChrisBAshton/smartresolution/builds/58932345
...
Let us look a little closer at the reason behind the design decision. The decision was made to make life easier for my controllers. Let us examine this with some pseudo-code:
...
Inside Dispute, we have something like:
...
Inside Dispute, we would now have something like this:
...
The advantage of the refactored solution is that it reduces the models' concept of the database layer, though there is still a difficulty in getting around this for persistent changes, such as \lstinline{$dispute->closeSuccessfully()}. It would also make testing easier, purer and more efficient, since we could pass in pure PHP arrays to constructors rather than relying on database retrieval.

Refactoring looks simple enough in the example above, but the design decision is prevalent throughout the codebase and is used in chaining. Again, some pseudo-code can be used to demonstrate the problem: in the below example, refactoring to pass arrays rather than ids would become very messy and require refactoring the names and types of accessor methods, not just constructors.

\begin{minipage}{\textwidth}
\begin{lstlisting}
$disputeID = getDisputeIDFromUrl();
$accountID = getAccountID();
$messages = getMessages($disputeID, $accountID);
\end{lstlisting}
\end{minipage}

This perhaps hints at a broader problem: object-relational mapping. It could be argued that at various points in my codebase I have not approached the problem in as object-oriented a way as I should have because I've had to `convert' the object to be in terms of the underlying relational database representation of it.
...
Capybara requires a browser driver such as Poltergeist to request and then render the webpage. The browser can be headless (running as a background process, hidden from view) or a window browser. Poltergeist, as the name might suggest, is headless. %https://github.com/teampoltergeist/poltergeist
...
Another important thing to monitor with each commit is code quality, something that CodeClimate is trying to automate. [REF] Whenever I push a commit to SmartResolution, CodeClimate queues another scan of the codebase and informs me on a level of 1-4 whether the quality of my codebase has improved or deteriorated, using metrics such as variable name length, code duplication, the number of possible paths through a block of code, and so on. Utilising thid third-party service helped fight the temptation to hack a bit of functionality in, instead encouraging good engineering practice and providing suggestions as to how the codebase could be made more maintainable. %https://codeclimate.com/github/ChrisBAshton/smartresolution

...
It could be argued that the first three of the original requirements were correctly identified and delivered, whereas the remaining two were surpassed by more pressing and relevant requirements, which were also delivered.
...
Modria is well suited to low-value e-commerce disputes. It provides a hosted platform which online retailers can become subscribers to, after which they can sign into their hosted area and view the disputes against their organisation.
...
What Modria cannot offer at this stage is a platform that is capable of concepts beyond e-commerce disputes and e-commerce terminology. Disputes come in all shapes and sizes, and  automated resolution suggestions for all of these can only be possible with a modular architecture.

SmartResolution provides this platform. It is open-source and comes with installation instructions, so unlike Modria it is not hosted. However, hosted SmartResolution instance on a commercial site (say, smartresolution.com rather than smartresolution.org), like WordPress does, is a possible avenue for commercial prosperity.

The important thing is not that it is open-source, but that it is extensible. SmartResolution supports arbitrary modules of business logic, allowing developers to define custom dispute types and heuristically-driven resolutions.

The platform comes tightly integrated with the SmartResolution Marketplace, giving developers an advertising platform and a means for distributing their modules. A large enough collection of quality modules - and a platform that supports the easy delivery of said modules - can be a catalyst for rapid growth, as demonstrated with Google's Android software, or Apple's iPhone and App Store.

I don't want to view SmartResolution through rose-tinted glasses - I don't think it is currently suitable for e-commerce distributors, or any dispute involving just two persons. However, for specialised disputes where the use of a lawyer is advised, SmartResolution offers something unique to the world of ODR.
...
Figure~\ref{uml:class} contains the class diagram showing the object-oriented nature of the system and the interaction between the models. I'm quite happy with the final design, but certain elements could be improved.

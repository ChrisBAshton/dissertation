\chapter{Comments: a Philosophy}
\label{appendix:comments}

Comments are often a contentious issue. \emph{Clean Code} (Robert Martin) has an entire chapter dedicated to them.~\cite{cleanCode} I think his and my opinion on comments can be summed up in one line, taken from that chapter: ``comments are, at best, a necessary evil." That is, they should be avoided where possible, only used ``to compensate for our failure to express ourself in code".
...
He wasn't as averse to comments as I've just made out: I know he used to put the odd comment in his code to clarify things for the reader. But he and I agree that a comment in your code is a code smell~\cite{appendix:codeSmell}; an indication that the code needs to be refactored to become more readable and remove the need for that comment.
